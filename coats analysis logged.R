v##  Replication of Heller and co 2014
...
mod1am <- lmer(f.salary %>% log ~ (1|ID) + tenure1, df.m)
mod1bm <- lmer(f.salary %>% log ~ (1|ID) + tenure1 + as.factor(year), df.m)
anova(mod1am, mod1bm) # F ratio test

##  Female F test for year given tenure
mod1af <- lmer(f.salary %>% log ~ (1|ID) + tenure1, df.f)
mod1bf <- lmer(f.salary %>% log ~ (1|ID) + tenure1 + as.factor(year), df.f)
anova(mod1af, mod1bf) # F ratio test

##  Robustness F tests
mod2am <- lmer(f.salary %>% log ~ (1|ID) + tenure2, df)
mod2bm <- lmer(f.salary %>% log ~ (1|ID) + tenure2 + as.factor(year), df)
anova(mod2am, mod2bm) # F ratio test

##  Step three: The effects of tenure over time ----


mod3am <- lmer(f.salary %>% log ~ (1|ID) + (1|year) + period + tenureband1 * tenure1, df.m)

mod3bm <- lmer(f.salary %>% log ~ (1|ID) + (1|year) + tenureband1 * period * tenure1, df.m)
anova(mod3am, mod3bm) #yes for tenure period effect

mod3af <- lmer(f.salary %>% log ~ (1|ID) + (1|year) + period + tenureband1 * tenure1, df.f)
mod3bf <- lmer(f.salary %>% log ~ (1|ID) + (1|year) + tenureband1 * period * tenure1, df.f)
...
##  Write out the model
summary(mod3bm)$coef %>% write.csv('./Results/Raw model results male logged.csv')
summary(mod3bf)$coef %>% write.csv('./Results/Raw model results female logged.csv')
...
int.tab(mod = mod3bm, x = 'tenure1', template.tab) %>% write.csv('./Results/Neater tenure male logged.csv')# 20+ for 1889 is just wrong
int.tab(mod = mod3bf, x = 'tenure1', template.tab) %>% write.csv('./Results/Neater tenure female ,logged.csv') # the 18889 results exist for female due to error they are not there

int.tab(mod = mod3bm, x = 'tenure1', template.tab)
